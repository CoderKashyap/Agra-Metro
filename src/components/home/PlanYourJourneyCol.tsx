...
const PlanYourJourneyCol = () => {
    ...
    const createFromStationsObjForDropdown = (stationName: string) => {
        ...
        if (stationName.length > 0) { setShowFromStation(true) } else { setShowFromStation(false); setFromStationObjRemake(stations) }
    }
                ...
                <form className="md:py-4">
                    ...
                    <div onClick={(e) => e.stopPropagation()} className="relative">
                        ...
                        {fromStation && fromStation ?
                            <div onClick={() => { setShowFromStation(!showFromStation); setShowToStation(false); setFromStationObjRemake(stations) }} className="flex items-center justify-between cursor-pointer shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 w-full p-1.5">
                                ...
                                <div onClick={(e) => { e.stopPropagation(); setFromStation(""); setShowToStation(false); setFromStationObjRemake(stations) }} className="rounded-full text-xs px-2 py-1.5 text-white bg-[#7ab5d0]"><span className="hidden md:block">Search</span> <span className="md:hidden block"><SearchIcon fontSize="small" /></span></div>
                            </div> : <>
                                <div onClick={() => { setShowToStation(false); setShowFromStation(!showFromStation) }} className="flex justify-center items-center">
                                    <input type="text" name="station" id="fromStation" className="shadow-sm border border-cyan-400 text-gray-800 sm:text-s rounded-md focus:outline-none focus:ring-0 focus:border-red-600 block w-full p-2" placeholder="Search station or click to select" onChange={(e) => createFromStationsObjForDropdown(e.target.value)} required />
                                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-6 ${showFromStation && "transform rotate-180"} absolute right-2 text-gray-600 cursor-pointer`} viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div onClick={(e) => e.stopPropagation()} className="relative my-4 md:my-6">
                        ...
                        {toStation && toStation ?
                            <div onClick={() => { setShowToStation(!showToStation); setShowFromStation(false); setToStationObjRemake(stations) }} className="flex items-center justify-between cursor-pointer shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 w-full p-1.5">
                                <div className="flex items-center">
                                ...
                                <div onClick={(e) => { e.stopPropagation(); setToStation(""); setShowFromStation(false); setToStationObjRemake(stations) }} className="rounded-full text-xs px-2 py-1.5 text-white bg-[#7ab5d0]"><span className="hidden md:block">Search</span> <span className="md:hidden block"><SearchIcon fontSize="small" /></span></div>
                            </div> : <>
                                <div onClick={() => { setShowToStation(!showToStation); setShowFromStation(false) }} className="flex justify-center items-center">
                                    <input type="text" name="toStation" id="toStation" className="shadow-sm border border-cyan-400 text-gray-900 sm:text-s rounded-md focus:outline-none focus:ring-0 focus:border-red-600 block w-full p-2" placeholder="Search station or click to select" onChange={(e) => createToStationsObjForDropdown(e.target.value)} required />
                                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-6 ${showToStation && "transform rotate-180"} absolute right-2 text-gray-600 cursor-pointer`} viewBox="0 0 20 20" fill="currentColor">
                                        <path fillRule="evenodd" d="M5.293 6.293a1 1 0 011.414 0L10 9.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
                                    </svg>
                                </div>
                            </>}
