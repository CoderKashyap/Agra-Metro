    ...
    return (
        <>
            <div className='flex flex-col gap-6'>
                ...
                <div onClick={() => { setShowFromStation(false) }} className="relative before:absolute before:bg-contain overflow-hidde before:h-40 before:w-52 before:top-4 sm:before:top-6 before:left-12 sm:before:left-20 before:opacity-10 before:bg-no-repeat before:bg-cente before:bg-[url('/watermarks/metro.png')] shadow-md bg-[#f5ffef] border border-[#b3d09f] rounded-lg p-4">
                    ...
                    <div onClick={(e) => e.stopPropagation()} className="my-3 relative">
                        {fromStation && fromStation ?
                            <div onClick={() => { setShowFromStation(!showFromStation); setFromStationObjRemake(stations) }} className="cursor-pointer flex items-center justify-between shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 w-full p-1.5">
                                ...
                                <div onClick={() => { setShowFromStation(!showFromStation) }} className="flex justify-center items-center">
                                    <input type="text" name="station" id="fromStation" className="shadow-sm text-gray-900 rounded-md focus:outline-none focus:ring-0 border border-gray-300 focus:border-gray-600 block w-full p-2" placeholder="Search station or click to select" onChange={(e) => createFromStationsObjForDropdown(e.target.value)} required />
                        ...
                        {showFromStation &&
                            <div className={`p-0 bg-[#fcfffa] border border-gray-300 absolute w-full z-10 rounded-b-lg overflow-y-auto h-auto max-h-68`}>


                                {fromStationObjRemake && fromStationObjRemake.map((station: { stationName: string }, index: number) => (
                                    <div onClick={() => { setFromStation(station.stationName); setShowFromStation(false); }} key={index} className="hover:bg-[#f1ffe7] rounded-lg px-2 py-1 relative flex gap-2 mx-2 my-2 items-center cursor-pointer">
                    ...
                    <div className="mt-4 flex justify-end items-center relative">
                        <button onClick={(e) => e.stopPropagation()} className="bg-[#02555b] cursor-pointer rounded-md text-white font-medium text-sm p-2 px-6">All Stations</button>
                    </div>
